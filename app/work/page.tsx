    ...
    <div className="min-h-screen flex items-center justify-center ">
      <main className="max-w-4xl m-3 md:m-8 z-50 w-full space-y-8 rounded-lg border border-xl border-gray-700/50 p-5 sm:p-10 backdrop-blur-xl bg-black/80">        
        <div className="relative flex justify-center" >
          <Link href="/">
            <Button variant="ghost" className="group hover:bg-gray-800/50 absolute left-0 hover:text-white rounded-full px-4 py-2 duration-300 cursor-pointer">
          ...
          {projects.map((project) => (
            <div key={project.id} className="group overflow-hidden rounded-lg border hover:scale-105 border-neutral-800/70 bg-black/50 transition-all duration-300">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-1 mb-4 overflow-x-auto max-w-full scrollbar-none">
                  {project.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="whitespace-nowrap px-2.5 py-0.5 text-[12px] rounded-md bg-slate-800/50 text-gray-300 border border-neutral-700"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="ghost" className="flex-1 text-white bg-black backdrop-blur-sm border-neutral-800/70 transition-colors duration-200 ease-linear" asChild>
                    <a href="">
